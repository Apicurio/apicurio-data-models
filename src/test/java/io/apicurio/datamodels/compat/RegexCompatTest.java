 ...
 */
public class RegexCompatTest {
    
    @Test
    public void testMatches() throws Exception {
        Assert.assertTrue(RegexCompat.matches("Hello World", ".+"));
        Assert.assertTrue(RegexCompat.matches("Hello World", "Hello.World"));
        Assert.assertTrue(RegexCompat.matches("Hello World", "H(.+).W(.*)"));
        Assert.assertFalse(RegexCompat.matches("Hello World", "H(.+).w(.*)"));
    }

    @Test
    public void testFindMatches() throws Exception {
        List<String[]> matches = RegexCompat.findMatches("this-is-some-test-data", "-([a-zA-Z]+)");
        ...
        matches = RegexCompat.findMatches("/path-1/bar-2/foo-33/baz-414/", "/([a-zA-Z]+)-([\\d]+)");
        Assert.assertEquals(4, matches.size());
    ...
    public void testPathSegmentMatching() throws Exception {
        ...
        List<String[]> matches = RegexCompat.findMatches("/pet", SEG_MATCH_REGEX);
        Assert.assertEquals(1, matches.size());
        
        matches = RegexCompat.findMatches("/pet/findByStatus", SEG_MATCH_REGEX);
        Assert.assertEquals(2, matches.size());

        matches = RegexCompat.findMatches("/pet/{petId}", SEG_MATCH_REGEX);
        Assert.assertEquals(2, matches.size());

        matches = RegexCompat.findMatches("/pet/{petId}/uploadImage", SEG_MATCH_REGEX);
        Assert.assertEquals(3, matches.size());
        
        matches = RegexCompat.findMatches("/store/order/{orderId}", SEG_MATCH_REGEX);
        Assert.assertEquals(3, matches.size());
        
        matches = RegexCompat.findMatches("/designs/{designId}/codegen/projects/{projectId}/zip", SEG_MATCH_REGEX);
