...
/**
 * Utilibility layer for marshalling and unmarshalling commands.
...
public class MarshallUtil {
     ...
     * @param command
     */
    public static ObjectNode marshallCommand(ICommand command) {
        ObjectNode rval = mapper.valueToTree(command);
        JsonUtil.setStringProperty(rval, "__type", command.type());
        return rval;
    }
     ...
     * @param from
     */
    public static ICommand unmarshallCommand(ObjectNode from) {
        String type = JsonUtil.consumeStringProperty(from, "__type");
        if (type == null) {
    ...
    private static ObjectNode marshallSimplifiedType(SimplifiedType sType) {
        if (NodeUtil.isNullOrUndefined(sType)) {
            return null;
        }
        ObjectNode obj = JsonUtil.objectNode();
        JsonUtil.setStringProperty(obj, "type", sType.type);
        // TODO convert from List<Object> to List<String> here rather than coercing it
        JsonUtil.setStringArrayProperty(obj, "enum", (List) sType.enum_);
        JsonUtil.setObjectProperty(obj, "of", marshallSimplifiedType(sType.of));
        JsonUtil.setStringProperty(obj, "as", sType.as);
        return obj;
    }
     ...
     */
    @SuppressWarnings({ "unchecked", "rawtypes" })
    private static SimplifiedType unmarshallSimplifiedType(ObjectNode from) {
        if (NodeUtil.isNullOrUndefined(from)) {
            return null;
        }
        SimplifiedType type = new SimplifiedType();
        type.type = JsonUtil.getStringProperty(from, "type");
        // TODO convert from List<String> to List<Object> here rather than coercing it
        type.enum_ = (List) JsonUtil.getStringArrayProperty(from, "enum");
        type.of = MarshallUtil.unmarshallSimplifiedType(JsonUtil.getObjectProperty(from, "of"));
        type.as = JsonUtil.getStringProperty(from, "as");
        return type;
    }
     ...
     * @param sType
     */
    private static ObjectNode marshallSimplifiedParameterType(SimplifiedParameterType sType) {
        if (NodeUtil.isNullOrUndefined(sType)) {
            return null;
        }
        ObjectNode obj = marshallSimplifiedType(sType);
        JsonUtil.setBooleanProperty(obj, "required", sType.required);
        return obj;
    }
     ...
     */
    @SuppressWarnings({ "unchecked", "rawtypes" })
    private static SimplifiedParameterType unmarshallSimplifiedParameterType(ObjectNode from) {
        if (NodeUtil.isNullOrUndefined(from)) {
            return null;
        }
        ...
        type.of = MarshallUtil.unmarshallSimplifiedType(JsonUtil.getObjectProperty(from, "of"));
        type.as = JsonUtil.getStringProperty(from, "as");
        Boolean req = JsonUtil.getBooleanProperty(from, "required");
        if (req != null) {
            type.required = req;
        }
        return type;
     ...
     * @param sType
     */
    private static ObjectNode marshallSimplifiedPropertyType(SimplifiedPropertyType sType) {
        if (NodeUtil.isNullOrUndefined(sType)) {
            return null;
        }
        ObjectNode obj = marshallSimplifiedType(sType);
     ...
     */
    @SuppressWarnings({ "unchecked", "rawtypes" })
    private static SimplifiedPropertyType unmarshallSimplifiedPropertyType(ObjectNode from) {
        if (NodeUtil.isNullOrUndefined(from)) {
            return null;
        }
    ...
    public static class NodePathDeserializer extends JsonDeserializer<NodePath> {
        ...
        public NodePath deserialize(JsonParser p, DeserializationContext ctxt)
            ...
            if (path != null) {
                rval = NodePathUtil.parseNodePath(path);
            }
            return rval;
        }
    }
    ...
    public static class SimplifiedTypeSerializer<T extends SimplifiedType> extends JsonSerializer<T> {
        ...
        public void serialize(T value, JsonGenerator gen, SerializerProvider serializers)
            ...
            if (this.flavor == SerializerFlavor.parameter) {
                c = MarshallUtil.marshallSimplifiedParameterType((SimplifiedParameterType)value);
            } else if (this.flavor == SerializerFlavor.property) {
                c = MarshallUtil.marshallSimplifiedPropertyType((SimplifiedPropertyType)value);
            } else {
                c = MarshallUtil.marshallSimplifiedType(value);
            }
    ...
    public static class SimplifiedTypeDeserializer<T extends SimplifiedType> extends JsonDeserializer<T> {
        ...
        public T deserialize(JsonParser p, DeserializationContext ctxt)
            ...
            ObjectNode value = (ObjectNode) deserializer.deserialize(p, ctxt);
            if (this.flavor == SerializerFlavor.property) {
                return (T) MarshallUtil.unmarshallSimplifiedPropertyType(value);
            } else if (this.flavor == SerializerFlavor.parameter) {
                return (T) MarshallUtil.unmarshallSimplifiedParameterType(value);
            } else {
                return (T) MarshallUtil.unmarshallSimplifiedType(value);
            }
        }
    }
        ...
        public void serialize(List<ICommand> value, JsonGenerator gen, SerializerProvider serializers)
            ...
            try {
                for (ICommand command : value) {
                    ObjectNode c = MarshallUtil.marshallCommand(command);
                    JsonSerializer<Object> cmdSerializer = serializers.findValueSerializer(c.getClass());
                    cmdSerializer.serialize(c, gen, serializers);
                }
        ...
        public List<ICommand> deserialize(JsonParser p, DeserializationContext ctxt)
            ...
            while (!done) {
                ...
                if (token == JsonToken.END_ARRAY) {
                    ...
                    JsonDeserializer<Object> deserializer = ctxt.findNonContextualValueDeserializer(SimpleType.constructUnsafe(JsonNode.class));
                    ObjectNode value = (ObjectNode) deserializer.deserialize(p, ctxt);
                    ICommand cmd = MarshallUtil.unmarshallCommand(value);
                    commands.add(cmd);
                }
            }
            return commands;
