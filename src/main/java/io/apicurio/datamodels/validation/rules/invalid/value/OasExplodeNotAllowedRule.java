 ...
 */
public class OasExplodeNotAllowedRule extends AbstractInvalidPropertyValueRule {
    ...
    public void visitEncoding(OpenApiEncoding node) {
        if (hasValue(node.isExplode())) {
            this.reportIf(!equals(mediaTypeName, "application/x-www-form-urlencoded"), node, "explode", map("name", mediaTypeName));
        }
    }
