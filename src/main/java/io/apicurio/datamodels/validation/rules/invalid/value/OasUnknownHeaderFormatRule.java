 ...
 */
public class OasUnknownHeaderFormatRule extends AbstractInvalidPropertyValueRule {

    /**
     * Constructor.
     * @param ruleInfo
     */
    public OasUnknownHeaderFormatRule(ValidationRuleMetaData ruleInfo) {
        super(ruleInfo);
    }
    ...
    public void visitHeader(OpenApiHeader node) {
        ...
        if (hasValue(header.getFormat())) {
            this.reportIfInvalid(isValidEnumItem(header.getFormat(), array("int32", "int64", "float", "double", "byte", "binary", "date", "date-time", "password")),
                    node, "format", map());
        }
    }
