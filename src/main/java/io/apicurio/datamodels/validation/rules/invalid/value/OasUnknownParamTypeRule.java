 ...
 */
public class OasUnknownParamTypeRule extends AbstractInvalidPropertyValueRule {

    /**
     * Constructor.
     * @param ruleInfo
     */
    public OasUnknownParamTypeRule(ValidationRuleMetaData ruleInfo) {
        super(ruleInfo);
    }
    ...
    public void visitParameter(Parameter node) {
        ...
        if (hasValue(param.getType())) {
            this.reportIfInvalid(isValidEnumItem(param.getType(), array("string", "number", "integer", "boolean", "array", "file")),
                    node, "type", map());
        }
    }
