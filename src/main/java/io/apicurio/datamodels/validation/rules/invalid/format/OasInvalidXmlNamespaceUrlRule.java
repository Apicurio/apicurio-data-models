 ...
 */
public class OasInvalidXmlNamespaceUrlRule extends ValidationRule {

    /**
     * Constructor.
     * @param ruleInfo
     */
    public OasInvalidXmlNamespaceUrlRule(ValidationRuleMetaData ruleInfo) {
        super(ruleInfo);
    }
    ...
    public void visitXML(OpenApiXML node) {
        if (hasValue(node.getNamespace())) {
            this.reportIfInvalid(isValidUrl(node.getNamespace()), node, "namespace", map());
        }
    }
