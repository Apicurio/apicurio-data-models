...
public class Oas20ReferenceLocalizationStrategy extends AbstractReferenceLocalizationStrategy implements ReferenceLocalizationStrategy {

    protected String PREFIX = "#/";

    @Override
    public Pair attachAsDefinition(Document model0, String name, Node component) {
        Oas20Document model = (Oas20Document) model0;
//        component._ownerDocument = model0;
        if (component instanceof Oas20Schema) {
            ...
            return new Pair(PREFIX + "definitions/" + name, definition);
        }
    ...
    public Map<String, Node> getExistingLocalComponents(Document model0) {
        Oas20Document model = (Oas20Document) model0; // TODO explicit check
        ...
        //transform(model.securityDefinitions.items, name -> PREFIX + "securityDefinitions/" + name, res);
