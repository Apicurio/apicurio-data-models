...
public class OasUnknownArrayTypeRule extends OasInvalidPropertyValueRule {
     ...
     * @param ruleInfo
     */
    public OasUnknownArrayTypeRule(ValidationRuleMetaData ruleInfo) {
        super(ruleInfo);
    }
    ...
    public void visitItems(Oas20Items node) {
        if (hasValue(node.type)) {
            this.reportIfInvalid(isValidEnumItem(node.type, array("string", "number", "integer", "boolean", "array")), 
