 ...
 */
public class OasUnexpectedHeaderCollectionFormatRule extends OasInvalidPropertyValueRule {

    /**
     * Constructor.
     * @param ruleInfo
     */
    public OasUnexpectedHeaderCollectionFormatRule(ValidationRuleMetaData ruleInfo) {
        super(ruleInfo);
    }
    ...
    public void visitHeader(OasHeader node) {
        ...
        if (hasValue(header.collectionFormat)) {
            this.reportIfInvalid(equals(header.type, "array"), node, Constants.PROP_COLLECTION_FORMAT, map());
        }
    }
