 ...
 */
public class OasExplodeNotAllowedRule extends OasInvalidPropertyValueRule {

    /**
     * Constructor.
     * @param ruleInfo
     */
    public OasExplodeNotAllowedRule(ValidationRuleMetaData ruleInfo) {
        super(ruleInfo);
    }
    ...
    public void visitEncoding(Oas30Encoding node) {
        if (hasValue(node.explode)) {
            Oas30MediaType mediaType = (Oas30MediaType) node.parent();
            this.reportIf(!equals(mediaType.getName(), "application/x-www-form-urlencoded"), node, Constants.PROP_EXPLODE, map("name", mediaType.getName()));
        }
    }
