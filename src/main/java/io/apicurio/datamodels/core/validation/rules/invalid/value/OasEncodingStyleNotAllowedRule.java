 ...
 */
public class OasEncodingStyleNotAllowedRule extends OasInvalidPropertyValueRule {

    /**
     * Constructor.
     * @param ruleInfo
     */
    public OasEncodingStyleNotAllowedRule(ValidationRuleMetaData ruleInfo) {
        super(ruleInfo);
    }
    ...
    public void visitEncoding(Oas30Encoding node) {
        if (hasValue(node.style)) {
            ...
            this.reportIfInvalid(mediaType.getName().indexOf("application/x-www-form-urlencoded") == 0, node, 
                    Constants.PROP_STYLE, map("name", mediaType.getName()));
        }
    }
