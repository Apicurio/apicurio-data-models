 ...
 */
public class OasUnexpectedHeaderUsageRule extends OasInvalidPropertyValueRule {
    ...
    public void visitEncoding(Oas30Encoding node) {
        if (node.getHeaders().size() > 0) {
            Oas30MediaType mediaType = (Oas30MediaType) node.parent();
            this.reportIfInvalid(mediaType.getName().indexOf("multipart") == 0, node, Constants.PROP_HEADERS, 
                    map("name", mediaType.getName()));
        }
    }
