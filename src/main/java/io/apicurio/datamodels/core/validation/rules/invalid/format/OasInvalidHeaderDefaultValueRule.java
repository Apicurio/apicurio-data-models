 ...
 */
public class OasInvalidHeaderDefaultValueRule extends ValidationRule {

    /**
     * Constructor.
     * @param ruleInfo
     */
    public OasInvalidHeaderDefaultValueRule(ValidationRuleMetaData ruleInfo) {
        super(ruleInfo);
    }
    ...
    public void visitHeader(OasHeader node) {
        ...
        if (hasValue(header.default_)) {
            this.reportIfInvalid(isValidForType(header.default_, header), header, Constants.PROP_DEFAULT, map());
        }
    }
