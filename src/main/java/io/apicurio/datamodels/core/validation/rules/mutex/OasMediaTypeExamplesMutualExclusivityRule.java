...
public class OasMediaTypeExamplesMutualExclusivityRule extends ValidationRule {
    ...
    /**
     * @see io.apicurio.datamodels.combined.visitors.CombinedAllNodeVisitor#visitMediaType(io.apicurio.datamodels.openapi.v3.models.Oas30MediaType)
     */
    @Override
    public void visitMediaType(Oas30MediaType node) {
        this.reportIf(hasValue(node.example) && node.getExamples().size() > 0, node, Constants.PROP_EXAMPLE, map());
    }
