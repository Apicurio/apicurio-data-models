...
public class OtEngine {
    ...
    public void executeCommand(OtCommand command, boolean pending) {
        if (pending) {
            command.local = true;
            LoggerUtil.info("[OtEngine] Executing PENDING command with contentId: %o", command.contentVersion);
        ...
        if (this.hasCommand(command)) {
            LoggerUtil.info("[OtEngine] Detected duplicate OtCommand with content version: %o", command.contentVersion);
            return;
        }

        LoggerUtil.info("[OtEngine] Executing command with content version: %o", command.contentVersion);
        int pidx;
    ...
    public void finalizeCommand(long pendingCommandId, long finalizedContentVersion) {
        LoggerUtil.info("[OtEngine] Finalizing command with contentId: %o  and new contentVersion: %o", pendingCommandId, finalizedContentVersion);
        ...
        if (isFirstPendingCmd && isLatestCmd) {
            LoggerUtil.info("[OtEngine] Pending command is 'next up', performing simple shift from pending to finalized.");
        ...
        if (found) {
            ...
            LoggerUtil.info("[OtEngine] Attempted to finalize pending command %d but was not found.", pendingCommandId);
        }
    ...
    public OtCommand undo(long contentVersion) {
        ...
        LoggerUtil.info("[OtEngine] Undo command with content version: %o", contentVersion);
    ...
    public OtCommand redo(long contentVersion) {
        ...
        LoggerUtil.info("[OtEngine] Redo command with content version: %o", contentVersion);
    ...
    protected void pruneRevertedCommands() {
        ...
        if (!cmds.isEmpty()) {
            LoggerUtil.info("[OtEngine] Pruned %d reverted commands", cmds.size());
        }
        ...
        if (!pcmds.isEmpty()) {
            LoggerUtil.info("[OtEngine] Pruned %d reverted pending commands", pcmds.size());
        }
    }
}
