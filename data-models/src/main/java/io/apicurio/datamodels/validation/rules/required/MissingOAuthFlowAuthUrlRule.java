...
public class MissingOAuthFlowAuthUrlRule extends RequiredPropertyValidationRule {
    ...
    public void visitOAuthFlow(OAuthFlow node) {
        ...
        if ("implicit".equals(flowType)) {
            this.requireProperty(node, "authorizationUrl", map("flowType", "Implicit"));
        }
        if ("authorizationCode".equals(flowType)) {
            this.requireProperty(node, "authorizationUrl", map("flowType", "Auth Code"));
        }
    }
