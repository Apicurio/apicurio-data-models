...
public class OasUnexpectedParamAllowEmptyValueRule extends AbstractInvalidPropertyValueRule {
    ...
    public void visitParameter(Parameter node) {
        ...
        if (hasValue(param.isAllowEmptyValue())) {
            if (param.root().modelType() == ModelType.OPENAPI20) {
                this.reportIfInvalid(isValidEnumItem(param.getIn(), array("query", "formData")), param, "allowEmptyValue",
                        map("options", "Query and Form Data"));
            } else {
                this.reportIfInvalid(isValidEnumItem(param.getIn(), array("query")), param, "allowEmptyValue",
                        map("options", "Query"));
            }
        }
    }
