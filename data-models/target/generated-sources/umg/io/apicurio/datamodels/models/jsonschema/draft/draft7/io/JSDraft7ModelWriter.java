...
public class JSDraft7ModelWriter implements ModelWriter {

	public void writeDocument(JSDraft7Document node, ObjectNode json) {
		if (node == null) {
			return;
		}
			...
			if (unionMap != null && !unionMap.isEmpty()) {
				...
				unionMap.keySet().forEach(key -> {
					...
					if (union.isJSchema()) {
						...
						this.writeJSchema((JSDraft7JSchema) union.asJSchema(), object);
		...
		}
		{
			...
			if (union != null) {
				...
				if (union.isJSchema()) {
					...
					this.writeJSchema((JSDraft7JSchema) union.asJSchema(), jsonValue);
			...
			if (union != null) {
				...
				if (union.isJSchemaList()) {
					...
					models.forEach(model -> {
						ObjectNode object = JsonUtil.objectNode();
						this.writeJSchema((JSDraft7JSchema) model, object);
						JsonUtil.addToArray(array, object);
					});
	...
	public ObjectNode writeRoot(RootNode node) {
		...
		this.writeDocument((JSDraft7Document) node, json);
		return json;
	}

	public void writeJSchema(JSDraft7JSchema node, ObjectNode json) {
		if (node == null) {
			return;
		}
