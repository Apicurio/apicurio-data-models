...
import io.apicurio.datamodels.models.jsonschema.draft.draft4.JSDraft4DocumentImpl;
...
public class JSDraft4ModelReader implements ModelReader {

	public void readDocument(ObjectNode json, JSDraft4Document node) {
		{
				...
				keys.forEach(key -> {
					...
					if (value != null) {
						if (JsonUtil.isObject(value)) {
							...
							JSDraft4JSchema model = (JSDraft4JSchema) node.createJSchema();
		...
		}
		{
			...
			if (value != null) {
				if (JsonUtil.isObject(value)) {
					...
					readJSchema(object, (JSDraft4JSchema) node.getNot());
				} else if (JsonUtil.isBoolean(value)) {
					Boolean pValue = JsonUtil.toBoolean(value);
					BooleanUnionValue unionValue = new BooleanUnionValueImpl(pValue);
		...
		}
		{
			...
			if (value != null) {
				if (JsonUtil.isObject(value)) {
					...
					readJSchema(object, (JSDraft4JSchema) node.getItems());
				} else if (JsonUtil.isArray(value)) {
					List<JsonNode> array = JsonUtil.toList(value);
					List<JSDraft4JSchema> models = new ArrayList<>();
					array.forEach(item -> {
		...
		}
		{
			...
			if (value != null) {
				if (JsonUtil.isObject(value)) {
					...
					readJSchema(object, (JSDraft4JSchema) node.getAdditionalItems());
				} else if (JsonUtil.isBoolean(value)) {
					Boolean pValue = JsonUtil.toBoolean(value);
					BooleanUnionValue unionValue = new BooleanUnionValueImpl(pValue);
		...
		}
		{
			...
			if (value != null) {
				if (JsonUtil.isObject(value)) {
					...
					readJSchema(object, (JSDraft4JSchema) node.getAdditionalProperties());
				} else if (JsonUtil.isBoolean(value)) {
					Boolean pValue = JsonUtil.toBoolean(value);
					BooleanUnionValue unionValue = new BooleanUnionValueImpl(pValue);
	...
	public RootNode readRoot(ObjectNode json) {
		JSDraft4Document rootModel = new JSDraft4DocumentImpl();
		this.readDocument(json, rootModel);
		return rootModel;
	}

	public void readJSchema(ObjectNode json, JSDraft4JSchema node) {
		{
