...
public class JS201909ModelWriter implements ModelWriter {

	public void writeDocument(JS201909Document node, ObjectNode json) {
		if (node == null) {
			return;
		}
		...
		JsonUtil.setStringProperty(json, "$recursiveRef", node.get$recursiveRef());
		JsonUtil.setBooleanProperty(json, "$recursiveAnchor", node.is$recursiveAnchor());
			...
			if (unionMap != null && !unionMap.isEmpty()) {
				...
				unionMap.keySet().forEach(key -> {
					...
					if (union.isJSchema()) {
						...
						this.writeJSchema((JS201909JSchema) union.asJSchema(), object);
		...
		}
		{
			...
			if (union != null) {
				...
				if (union.isJSchema()) {
					...
					this.writeJSchema((JS201909JSchema) union.asJSchema(), jsonValue);
			...
			if (union != null) {
				...
				if (union.isJSchemaList()) {
					...
					models.forEach(model -> {
						ObjectNode object = JsonUtil.objectNode();
						this.writeJSchema((JS201909JSchema) model, object);
						JsonUtil.addToArray(array, object);
					});
	...
	public ObjectNode writeRoot(RootNode node) {
		...
		this.writeDocument((JS201909Document) node, json);
		return json;
	}

	public void writeJSchema(JS201909JSchema node, ObjectNode json) {
		if (node == null) {
			return;
		}
